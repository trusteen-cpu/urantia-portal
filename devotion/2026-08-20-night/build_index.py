...
var A=new Audio(),cur=0,playing=false,repMode=0; /* 0 전체 1 이 메뉴 2 없음 */
...
function load(i,go){
  cur=i;mark();
  A.src=DATA.audioBase+FILES[i]+'.mp3';
...
A.addEventListener('error',function(){if(playing)nextSeg()});  /* 노래 없으면 넘어감 */
...
mark();A.src=DATA.audioBase+FILES[0]+'.mp3';
</script></body></html>
"""
