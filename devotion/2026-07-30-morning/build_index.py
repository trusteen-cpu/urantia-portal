...
body{
  ...
  color:var(--ink);
  background:linear-gradient(180deg,#161d3a 0%,#262b52 30%,#4a3f63 55%,#9a6a56 78%,#e8a860 100%);background-attachment:fixed;
  min-height:100%;
}
#sky{position:fixed;inset:0;z-index:0}
.sun{
  position:fixed;bottom:-140px;right:-60px;width:420px;height:420px;border-radius:50%;
  background:radial-gradient(circle at 50% 45%,#fff3d6,#f6c37a 45%,rgba(232,168,96,.35) 70%,rgba(232,168,96,0) 78%);
  filter:blur(2px);
...
.player{
  ...
  background:rgba(14,18,38,.86);backdrop-filter:blur(10px);
...
@media (max-width:520px){
  ...
  .sun{width:300px;height:300px;bottom:-110px;right:-70px}
...
(function(){
  ...
  function init(){
    ...
    stars = [];
    for(let i=0;i<70;i++){
      const y = Math.random()*cv.height*0.58;
      stars.push({
  ...
  function draw(){
    ...
    for(const st of stars){
      st.a += st.s;
      const alpha = (0.30 + Math.abs(Math.sin(st.a))*0.50) * st.f;
      ctx.beginPath();
